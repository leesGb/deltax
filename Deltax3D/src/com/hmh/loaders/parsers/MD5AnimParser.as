package com.hmh.loaders.parsers
{
	...
	public class MD5AnimParser extends ParserBase
	{
		...
		public var _clip:Vector.<SkeletonPose>;
		...
		{
			/*
			additionalRotationAxis = Vector3D.Z_AXIS;
			additionalRotationRadians =  -Math.PI * .5;
			*/
			super("plainText");
			...
			t3.fromAxisAngle(Vector3D.Z_AXIS, -Math.PI);
			//_rotationQuat.multiply(t2, t1);
			//_rotationQuat.multiply(t3,_rotationQuat);
		...
		private function translateClip() : void
		{
			for (var i : int = 0; i < _numFrames; ++i)
				_clip.push(translatePose(_frameData[i]));
		}
